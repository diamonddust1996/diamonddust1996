...
Module OW_Module
    Public Sub OW_Trans(ByVal strFile As String, ByVal sTypeData As String, ByVal sData As String, ByVal sTrans As String)
        ...
        Do While Row < 64
            ...
            If ExlSheet.Range("B" & Row).Value <> "" Then
                '品種
                sType = Split(ExlSheet.Range("B" & Row).Value, "_")(0)
                If Left(Split(ExlSheet.Range("B" & Row).Value, "_")(1), 1) = "R" Or Left(Split(ExlSheet.Range("B" & Row).Value, "_")(1), 1) = "T" Or Left(Left(Split(ExlSheet.Range("B" & Row).Value, "_")(1), 1), 1) = "W" Or Left(Left(Split(ExlSheet.Range("B" & Row).Value, "_")(1), 1), 1) = "9" Then
                    'Lot
                    sLot = Left(Split(ExlSheet.Range("B" & Row).Value, "_")(1), 4)
                    'ROLL
                    sRoll = Mid(Split(ExlSheet.Range("B" & Row).Value, "_")(1), 5, Len(Split(ExlSheet.Range("B" & Row).Value, "_")(1)) - Len(sLot))

                Else
                    'Lot
                    sLot = Left(Split(ExlSheet.Range("B" & Row).Value, "_")(1), 3)
                    'ROLL
                    sRoll = Mid(Split(ExlSheet.Range("B" & Row).Value, "_")(1), 4, Len(Split(ExlSheet.Range("B" & Row).Value, "_")(1)) - Len(sLot))
                End If
                'SLIT
                sSlit = Left(Split(ExlSheet.Range("B" & Row).Value, "_")(2), 2)
                'SET
                If InStr(Right(ExlSheet.Range("B" & Row).Value, 2), "-") > 0 Then

                    sSet = Left(Right(ExlSheet.Range("B" & Row).Value, 4), 2)
                    sSet_Name = Right(ExlSheet.Range("B" & Row).Value, 2)

                Else

                    sSet = Right(ExlSheet.Range("B" & Row).Value, 2)
                    sSet_Name = ""
                End If

                If InStr(ExlSheet.Range("B" & Row).Value, "REF") >= 1 Then
                    sSet_Name = Mid(Split(ExlSheet.Range("B" & Row).Value, "_")(2), 4)
                    sSet = Left(Mid(Split(ExlSheet.Range("B" & Row).Value, "_")(2), 4), 2)
                End If
                '要変フラグ
                sY_Flg = "F"
                '要変工程フラグ
                ...
                If sSet_Name <> "" And InStr(sSet_Name, "REF") = 0 Then
                    y = y + 1
                    If InStr(ExlSheet.Range("B" & Row + 1).Value, sLot & sRoll & "_" & sSlit & " " & sSet) = 0 And InStr(ExlSheet.Range("B" & Row - 1).Value, sLot & sRoll & "_" & sSlit & " " & sSet) = 0 Then
                        y = 0
                    End If
                ...
                sAMP = ExlSheet.Range("D" & Row).Value

                sSNRsk_L = Split(sData, ",")(1)

                sSNRsk = ExlSheet.Range("F" & Row).Value

                sBBSNR_L = Split(sData, ",")(2)

                sBBSNR = ExlSheet.Range("H" & Row).Value

                sOW_U = Split(sData, ",")(3)

                sOW = ExlSheet.Range("J" & Row).Value

                sRes_U = Split(Split(sData, ",")(4), "～")(1)

                sRes_L = Split(Split(sData, ",")(4), "～")(0)

                sRes = ExlSheet.Range("L" & Row).Value

                sORC_U = Split(Split(sData, ",")(5), "～")(1)

                sORC_L = Split(Split(sData, ",")(5), "～")(0)

                sORC = ExlSheet.Range("N" & Row).Value
