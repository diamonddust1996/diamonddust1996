...
Public Class Main
    ...
    Private Sub Main_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        ...
        DataGridView.Rows.Add(True, "C:\temp", "MP", "CHOU", "Gen6", "CSB", "123456--1", "準備完了", "QA04")
        DataGridView.Rows.Add(True, "C:\temp", "MP", "CHOU", "Gen5", "CPA", "123456--1", "準備完了", "QA04")
    ...
    Private Sub ExitButton_Click(ByVal sender As Object, ByVal e As EventArgs) Handles ClearButton.Click
        MsgBox(DataGridView.Rows(0).Cells(2).Value)
    End Sub
    ...
    Private Sub StartButton_Click(ByVal sender As Object, ByVal e As EventArgs) Handles StartButton.Click
        ...
        If DropFileBox.Text = "Please Drop ExcelFile or Folder" Or DropFileBox.Text = "ExcelFile is not found!" Then
            'ファイルが存在しない時に表示する
            DropFileBox.Text = "ExcelFile is not found!"
            Exit Sub
    ...
    Private Sub SelectAll_ToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SelectAll_ToolStripMenuItem.Click
        For i As Integer = 0 To DataGridView.RowCount - 1
            If DataGridView.Rows(i).Visible = True Then
    ...
    Private Sub DataGridView_CellMouseClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles DataGridView.CellMouseClick
        ...
        If e.Button = MouseButtons.Right Then
            ...
            For i As Integer = 0 To DataGridView.Rows.Count - 1
                If i = 0 Then DataGridView_ContextMenuStrip.Items.Add(MenuSeparator)

                If Not DataGridView.Rows(i).Cells(e.ColumnIndex).Value = "" Then

                    If DataGridView_ContextMenuStrip.Items.Count = 3 Then
                        DataGridView_ContextMenuStrip.Items.Add(DataGridView.Rows(i).Cells(e.ColumnIndex).Value)

                    Else
                        AddFlag = True
                        For j As Integer = 3 To DataGridView_ContextMenuStrip.Items.Count - 1
                            If DataGridView.Rows(i).Cells(e.ColumnIndex).Value = DataGridView_ContextMenuStrip.Items(j).Text Then
                                AddFlag = False
                                Exit For
                            End If
                        Next
                        If AddFlag = True Then DataGridView_ContextMenuStrip.Items.Add(DataGridView.Rows(i).Cells(e.ColumnIndex).Value)
                    End If

                    End If
            Next
            DataGridView_ContextMenuStrip.Show(Cursor.Position)
        End If
    End Sub
End Class